namespace DatesAndStuff.Tests
{
    public sealed class SimulationTimeTests
    {
        ...
        public void SimulationTime_NotEqualsOperator_ShouldReturnFalseForEqualInstances()
        {
            ...
            // Act
            // Assert
            Assert.IsFalse(sut1 != sut2, "Should return false for equal instances");
        }
        ...
        {
            var sut = new SimulationTime(2026, 1, 1);
            Assert.AreEqual(sut.TotalMilliseconds + 1, sut.NextMillisec.TotalMilliseconds);
        }
        ...
        {
            var dt = new DateTime(2026, 1, 1, 12, 0, 0);
            var sut = new SimulationTime(dt);
            Assert.AreEqual(dt.AddMilliseconds(500), sut.AddMilliseconds(500).ToAbsoluteDateTime());
        }
        ...
        public void GiveDateTimeAndSimulationTime_WhenAddingSeconds_ThenTimesShouldBeEqual()
        {
            var dt = new DateTime(2026, 1, 1, 12, 0, 0);
            var sut = new SimulationTime(dt);
            Assert.AreEqual(dt.AddSeconds(5), sut.AddSeconds(5).ToAbsoluteDateTime());
        }
        ...
        [Test]
        public void SimulationTime_AddDays_ShouldShiftTimeCorrectly()
        {
            // Arrange
            var baseDate = new DateTime(2026, 4, 29, 12, 0, 0);
            var sut = new SimulationTime(baseDate);

            // Act
            var result = sut.AddDays(2);

            // Assert
            Assert.AreEqual(baseDate.AddDays(2), result.ToAbsoluteDateTime());
        }
    }
}